...
class CSVWrapper(object):
    ...
    def loadProfiles(filePath, startIDFrom=0, separator=",", header=True, realIDField="", sourceId=0):
        ...
        def rowToAttributes(row):
            #a = filter(lambda k: not k.isEmpty(), [KeyValue(col, row[col]) for col in columnNames])
            #c = list(a)
            #print("***********")
            #print(c[0])
            #print(c[1])
...
#            print("realIDField", realIDField)
            ...
            if len(realIDField) > 0:
                ...
                id = []
                for i in attr:
                    if i.key == realIDField:
                        id.append(i.value)
                    else:
                        attr_.append(i)

                realID = id[0]
              #  realID = "".join(map(lambda a: a.value, filter(lambda a: a.key == "id", attributes)))#.strip()

            #return Profile(profileID, list(filter(lambda a: a.key != realIDField, attributes)), realID, sourceId)
            return Profile(profileID, attr_, realID, sourceId)

        #a = df.rdd.map(lambda row: rowToAttributes(row)).zipWithIndex().map(lambda x: getProfile(x))
        #print("**************************************")
        #print("a", a)
        #c = (list(a.collect()))
        #for i in c:
