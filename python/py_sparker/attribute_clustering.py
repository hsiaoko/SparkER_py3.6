...
class AttributeClustering(object):
    ...
    def getHashes(strHash, numHashes, seed=1234):
        ...
        def getPart():
            ...
            return int(((a * long(strHash) + b) % 2147495899) % sys.maxint)
    ...
    @staticmethod
    def clusterSimilarAttributes(profiles, numHashes, targetThreshold, maxFactor=1.0, numBands=-1, keysToExclude=[],
                                 computeEntropy=False, separator="_", normalizeEntropy=False):
        """
        ...
        def getTokens(profile):
            ...
            split = map(lambda a: (
            a.key, filter(lambda t: len(t) > 0, map(lambda t: t.strip(), re.split('\W+', a.value.lower())))),
                        attributes)
        ...
        attributesPerToken = attributesToken.map(lambda a: (a[1], a[0])).groupByKey().zipWithIndex().map(
            lambda x: (x[1], x[0][1]))
        ...
        hashes = sc.broadcast(
            attributesPerToken.map(lambda a: (a[0], AttributeClustering.getHashes(a[0], numHashes))).collectAsMap())

        # Generates an RDD of (attribute, [tokenIDs])
        tokensPerAttribute = attributesPerToken.flatMap(lambda a: [(p, a[0]) for p in a[1]]).groupByKey().map(
        ...
        def getEdges(cluster):
            ...
            return map(lambda p: (p[0], (p[1],
                                         AttributeClustering.calcSimilarity(attributeSignaturesBroadcast.value[p[0]],
                                                                            attributeSignaturesBroadcast.value[p[1]]))),
                       gpairs)
        ...
        edgesPerKey = edges.union(edges.map(lambda e: (e[1][0], (e[0], e[1][1])))).groupByKey().map(
            lambda x: (x[0], set(x[1])))
        ...
        nonClusteredAttributes = map(lambda k: str(k.sourceName) + str(separator) + str(k.attribute),
                                     filter(lambda a: a not in clusteredAttributes, allAttributes.collect()))
        ...
        if computeEntropy:
            ...
            def getAttributeEntropy(data):
                attribute = data[0]
                tokens = data[1]
                numberOfTokens = float(len(tokens))  # type: float
                c = Counter(tokens)
            ...
            entropyPerCluster = entropyPerAttribute.map(associateEntropyToCluster).groupByKey().map(
            ...
            defaultCluster = KeysCluster(defaultClusterID, nonClusteredAttributes,
                                         entropyMap.get(defaultClusterID, 1.0))
            ...
            # Put all together, and generates the final clusters.
            finalClusters = list(map(createCluster, clusters))
            finalClusters.append(defaultCluster)
